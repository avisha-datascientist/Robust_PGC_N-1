...
class TrainAgent_PDC(object):
    def __init__(self, agent, env, test_env, device, dn_json_path, serialized_agent_dir, dn_ffw, 
                 ep_infos, episode=None):
        self.device = device
        self.agent = agent
        self.env = env
        ...
        self.actor_id = 0
        self.log_alpha = self.agent.log_alpha
        self.disturbance = self.agent.disturbance
        self.emb = self.agent.emb
        self.Q = self.agent.Q
    ...
    def interaction(self, obs, prev_act, cid, ffw, start_step):
        ...
        while True:
            ...
            # obs.line_status[2] = False
            # print(obs.line_status)
            # self.env.backend.update_from_obs(obs)
            # self.env.backend.next_grid_state()
            ...
            step += 1
            self.total_actor_steps += 1

            # granular level rewards
            self.writer.add_scalar('Loss Reward per time step', train_reward, global_step=self.total_actor_steps)
            self.writer.add_scalar('Reward per time step', reward, global_step=self.total_actor_steps)

            # aux_add_data: to update episode data per step -- add here
            # which is basically calling episode.incr_store
        ...
        # wandb.log({'train_reward': train_reward, 'step': step})
    ...
    def run_actor_workers(self, actor_id, seed, train_chronics, max_ffw, output_dir, model_name, shared_memory, chronic_priority, shared_weights, learning_end):
        self.time = time()

        load = False
        while load is False:
            load = CommonFunctions.load_weights()
        ...
        CommonFunctions.save_chronic_priority(chronic_records)

        self.actor_id = actor_id
        self.shared_memory = shared_memory
        self.shared_weights = shared_weights
        self.chronic_priority = chronic_priority
        self.learning_end = learning_end
        print(self.chronic_priority)

        output_result_dir = os.path.join(output_dir, model_name)
        summary_dir = os.path.join(
            output_result_dir, 'summary', f'actor-{self.actor_id}')
        if not os.path.exists(summary_dir):
            os.makedirs(summary_dir)
        self.writer = SummaryWriter(log_dir=summary_dir) 

        # training loop
        # nb_frame is very large, if increments are happening once per chronic reduce it to reduce training time.
        # this while loop samples different chronics(same one multiple times) and for each selected runs an episode.
        while CommonFunctions.load_learning_end_flag():
            # load common chronic_priority in chronic records
            chronic_records = CommonFunctions.load_chronic_priority()
            ...
            # already done: self.env.seed(seed)

            ## set seeds
            ep_alive_frame = self.train_one_episode(seed, chronic_id, ffw)
            chronic_records[record_idx] = self.chronic_priority(chronic_id, ffw, ep_alive_frame)
            # not used further: chronic_step_records[record_idx] = alive_frame
            self.interval(chronic_records)
            # change chronic priority in the common chronic_priority queue

    
    def train_one_episode(self, seed, chronic_id, ffw):
        # first thing to implement loading or saving to shared_memory
        ...
        while not done:
            ...
            end_ = time.perf_counter()
                ...
                # manual updates to actions,rewards etc.
                
            alive_frame, prev_act = info[1][:2]
                # episode.set_game_over(game_over_step = alive_frame) # Does not work for some reason TODO!!
            total_reward += reward
            train_reward += info[0][3]
               
            self.episode.set_meta(self.env, alive_frame, float(total_reward), seed, None)
            # save in local worker memory
            self.memory.append(list(map(lambda x: x.cpu() if torch.is_tensor(x) else x, info[0])))
            if len(self.memory) == self.agent.k_step or done:
                for transition in self.multi_step_transition(self.memory):
                    CommonFunctions.append_shared_sample(*transition)
                    # update the number of tranisitions stored in sharedmemory,
                # if number greater than self.agent.update_step, then start updating another counter
                self.memory.clear()

            if self.shared_memory.qsize() > self.agent.update_start:
                CommonFunctions.incr_learnings_num()

            if CommonFunctions.load_learning_end_flag() == False:
                break
        ...
        t_time = end_ - beg_
        self.writer.add_scalar('Cumulative reward', total_reward, global_step=self.episode_num)
        self.writer.add_scalar('Cumulative Loss reward', train_reward, global_step=self.episode_num)
        self.writer.add_scalar('Agent Time to Act', time_act, global_step=self.episode_num)
        self.writer.add_scalar('Total Episode Time', t_time, global_step=self.episode_num)
            # is add_scalar correct for alive frame
        self.writer.add_scalar('Episode Alive Frame', alive_frame, global_step=self.episode_num)

        self.episode.set_episode_times(self.env, time_act, beg_, end_)
            #os.chown(self.serialized_agent_dir, os.stat(self.serialized_agent_dir).st_uid)
        self.episode.to_disk()

        return alive_frame
        
    def interval(self, chronic_records):
        CommonFunctions.save_chronic_priority(chronic_records)
        
        if self.episode_num % self.model_load_interval == 0:
            CommonFunctions.load_weights()
        #if self.episode_num % self.memory_save_interval == 0:
            #self.save_memory()
        ...
        while self.agent.update_step < nb_frame:
            ...
            while not done:
                ...
                if self.agent.update_step > nb_frame :
                    break
            global_timestep_count += alive_frame 
            ...
            t_time = end_ - beg_
            self.writer.add_scalar('Cumulative reward', total_reward, global_step=global_episode_count)
            self.writer.add_scalar('Cumulative Loss reward', train_reward, global_step=global_episode_count)
            self.writer.add_scalar('Agent Time to Act', time_act, global_step=global_episode_count)
            self.writer.add_scalar('Total Episode Time', t_time, global_step=global_episode_count)
            # is add_scalar correct for alive frame
            self.writer.add_scalar('Episode Alive Frame', alive_frame, global_step=global_episode_count)
