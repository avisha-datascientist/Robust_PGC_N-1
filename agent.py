...
class Agent(BaseAgent):
    def __init__(self, env, cversion_path, **kwargs):
        self.device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        self.obs_space = env.observation_space
        self.action_space = env.action_space
        super(Agent, self).__init__(env.action_space)
        self.cversion_path = cversion_path
        ...
        self.delay_step = 2
        self.update_step = 0
        self.update_current_version = 50
        self.k_step = 1
        ...
        self.run_num = kwargs.get('run_num', 1)
        if self.run_num > 1:
    ...
    def stack_obs(self, obs):
        ...
        if len(obs_vect) < 300:
            ...
            if len(obs_vect) < 600:
                obs_vect = obs_vect[:420]
            else:
                obs_vect = obs_vect[:1266]
    ...
    def update(self, writer):
        ...
        with torch.no_grad():
            ...
            perturbed_tstates2 = tstates2 + self.disturbance
            ...
            targets = self.tQ.min_Q(perturbed_tstates2, critic_input2, adj2, order2) - self.log_alpha.exp() * log_pi2
       
        def func(x, c_input, adj, order):
            return self.tQ.min_Q(x, c_input, adj, order) - self.log_alpha * log_pi2
        
        jacobian_matrix = torch.autograd.functional.jacobian(lambda perturbed_tstates2_: func(perturbed_tstates2_, critic_input2, adj2, order2), perturbed_tstates2)

        # below initialization needs to be uncommented as required
        # WCCI 2020:
        #jacobians = torch.empty((0, 177, 128), dtype = torch.float32).to(self.device)
        
        #IEEE-14
        #jacobians = torch.empty((0, 57, 128), dtype = torch.float32).to(self.device)

        #IEEE-5
        jacobians = torch.empty((0, 21, 128), dtype = torch.float32).to(self.device)
        for i in range(0, self.batch_size):
            data = jacobian_matrix[i, :, i, :, :]
            jacobians = torch.cat((jacobians, data), 0)

        jacobians_1d = jacobians.view(torch.numel(jacobians))
        jacobians_norm = jacobians_1d.pow(2).sum().sqrt().to(self.device)
       
        dist_ratio = (self.epsilon/jacobians_norm).to(self.device)
        ...
        self.Q.eval()
        writer.add_scalar('Epsilon By Norm Jacobians', dist_ratio, global_step=self.update_step)
        writer.add_scalar('Loss/Train Q1', Q1_loss, global_step=self.update_step)
        writer.add_scalar('Loss/Train Q2', Q2_loss, global_step=self.update_step)
        writer.add_scalar('Loss/Train Total', loss, global_step=self.update_step)
        ...
        if self.update_step % self.delay_step == 0:
            ...
            writer.add_scalar('Loss/Train Actor', actor_loss, global_step=eval_iter)
            writer.add_scalar('Loss/Train Alpha', alpha_loss, global_step=eval_iter)
        self.emb.eval()

        return predQ1.detach().mean().item(), predQ2.detach().mean().item()
    
    def save_most_recent(self):
        print("saving for learning number:", self.update_step)
        # disturbance
        torch.save(self.disturbance, os.path.join(self.cversion_path, f'disturbance.pt'))
        
        # replay buffer
        self.memory.store_memory(self.cversion_path)
        # update_num save as numpy array
        np.save(os.path.join(self.cversion_path, f'learnings_num.npy') , np.array(self.update_step))
        # model save
        torch.save(self.actor.state_dict(), os.path.join(self.cversion_path, f'actor.pt'))
        torch.save(self.emb.state_dict(), os.path.join(self.cversion_path, f'emb.pt'))
        torch.save(self.Q.state_dict(), os.path.join(self.cversion_path, f'Q.pt'))
        # log_alpha
        torch.save(self.log_alpha,  os.path.join(self.cversion_path, f'logalpha.pt'))
        

    def load_previous_version(self):
        self.disturbance = torch.load(os.path.join(self.cversion_path, f'disturbance.pt'), map_location=self.device)
        self.update_step = np.load(os.path.join(self.cversion_path, f'learnings_num.npy'))
        self.memory.extend_memory(self.cversion_path)

        self.actor.load_state_dict(torch.load(os.path.join(self.cversion_path, f'actor.pt'), map_location=self.device))
        self.emb.load_state_dict(torch.load(os.path.join(self.cversion_path, f'emb.pt'), map_location=self.device))
        self.Q.load_state_dict(torch.load(os.path.join(self.cversion_path, f'Q.pt'), map_location=self.device))
        self.log_alpha = torch.load(os.path.join(self.cversion_path, f'logalpha.pt'), map_location=self.device)
